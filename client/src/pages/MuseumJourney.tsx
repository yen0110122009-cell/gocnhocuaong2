...
export default function MuseumJourney({ account, profile, config }: { account: StudyAccount; profile: ProfileState; config: AppConfig }) {
  return <>
    ...
    {config.characters.length > 0 && <section className="panel mt-7 p-6" aria-labelledby="fragment-map-title" aria-describedby="fragment-map-status">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-amber-700 dark:text-amber-300">Bộ sưu tập mảnh ghép</p><h2 id="fragment-map-title" className="mt-2 font-display text-2xl font-bold">Ghép từng mảnh, mở từng câu chuyện</h2><p id="fragment-map-status" role="status" aria-live="polite" className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Mỗi ô đại diện cho mảnh ghép đã có hoặc còn thiếu của một nhân vật. Khi số mảnh đạt mốc do quản trị viên đặt, tiểu sử và timeline được mở trong Bảo tàng.</p></div><span className="inline-flex items-center gap-2 rounded-full bg-amber-100 px-3 py-2 text-sm font-bold text-amber-900 dark:bg-amber-500/15 dark:text-amber-100"><Puzzle className="h-4 w-4" />{Object.values(profile.fragments).reduce((sum, value) => sum + Math.max(0, value), 0)} mảnh đang giữ</span></div>
