...
| Piece/fragment | Đã có schema `piece_types`, `user_pieces`, `piece_transactions`, atomic ledger service, idempotency và audit boundary | Exchange rules, database/concurrency cloud verification và migration apply thực tế vẫn chờ credentials. |
...
| MB-03 | P0 | Piece ledger | **Đã triển khai atomic server mutation**, conditional non-negative update, unique idempotency key, audit write và Admin/Founder tRPC adjustment/balance boundary; exchange rules chưa triển khai. | Contract tests, typecheck/build đạt; cloud transaction/concurrency và migration apply thực tế còn chờ DATABASE_URL. |
...
Batch hiện tại đã hoàn tất schema/rule layer của MB-01/MB-02 và triển khai atomic piece ledger service cho MB-03. Contract tests, full regression, typecheck và build đều đạt; bước kế tiếp vẫn cần DATABASE_URL để apply/rollback migration và xác nhận transaction/concurrency cloud. Không tuyên bố cloud production readiness từ test local.
